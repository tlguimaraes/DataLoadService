...
cd ../complete
mvn clean package
java -jar target/DataLoadService-0.1.0.jar &
PID=$!
sleep 15
curl -s http://localhost:8080/dataload > target/actual.json
curl -s http://localhost:8080/dataload-javaconfig > target/actual-javaconfig.json
kill -9 $PID
...
./gradlew clean build
java -jar build/libs/DataLoadService-0.1.0.jar &
PID=$!
sleep 15
curl -s http://localhost:8080/dataload > build/actual.json
curl -s http://localhost:8080/dataload-javaconfig > build/actual-javaconfig.json
kill -9 $PID
